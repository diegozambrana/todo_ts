...
export const ToDoProvider =(props: any) => {
  ...
  const {
    item: todoData,
    saveItem: setTodoData
  } = useLocalStorage('TODO_REACT_TS', []);
  ...
  const onCheckTask = (idTask: string) => {
    const newTodoData = todoData.map((task: TaskType) => task.id === idTask
  ...
  const onCheckStep = (idTask: string, idStep: string) => {
    const newTodoData = todoData.map((task: TaskType) => task.id === idTask ? {
      ... task,
      steps: task.steps.map((step: StepType) => step.id === idStep
